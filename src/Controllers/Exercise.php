...
class Exercise {
    ...
    public function print_it($conn, $is_featured=false, $is_mine=false, $construct=false, $is_current=false){ // print exercise card
        ...
        if ($construct){ // for construct pages
            $button = '<button class="button-text exercise-item__add" name="add" value="'.$this->id. '" type="button"><p>Добавить</p> <img src="../../assets/img/add.svg" alt=""></button>';
        }else{
            if ($is_mine){ // if mine
                $button = '<button class="button-text exercise-item__delite" name="delete" value="'.$this->id. '"><p>Удалить</p> <img src="../../assets/img/delete.svg" alt=""></button>';
            }else{
                $button = '<button class="button-text exercise-item__add" name="add" value="'.$this->id. '"><p>Добавить</p> <img src="../../assets/img/add.svg" alt=""></button>';
            }
        }

        if ($is_featured){ // if featured
            $button_featured = '<button class="exercise-item__favorite exercise-item__favorite--selected" name="featured" value="'.$this->id. '"><img src="../../assets/img/favorite_added.svg" alt=""></button>';
        }else{
            $button_featured = '<button class="exercise-item__favorite" name="featured" value="'.$this->id. '"><img src="../../assets/img/favorite.svg" alt=""></button>';
        }

        $muscle_list = "";
        foreach ($this->muscles as $muscle){
            $muscle_list .= translate_group($muscle) . " Exercise.php"; // Translate each muscle name and concatenate them with spaces
        }
...
class User_Exercise extends Exercise { // class of exercise with reps and approaches
    ...
    public function print_control_exercise($conn, $is_featured=false, $current=false, $construct=false){ // same as print_it function, but get control exercise data and render control exercise template
        ...
        $muscle_list = "";
        foreach ($this->muscles as $muscle){
            $muscle_list .= translate_group($muscle) . " Exercise.php";
        }
