...
class User {
    ...
    public function redirect_logged($way=''){ // regirect to profile of user
        if ($this->auth){
            header("Location: ".$way."src/Pages/profile.php"); // regirect to profile page
        }
    }
    ...
    public function authenticate($conn, $login, $password){
        ...
        header('Location: src/Pages/profile.php'); // Redirect to user profile
    }
    ...
    public function get_control_workouts($conn, $is_done, $user_id=NULL){ // get control workouts list
        $sql = NULL;
        switch ($this->get_status()) {
            case "coach":
