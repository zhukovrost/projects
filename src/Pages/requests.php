...
$requests = $user_data->get_requests($conn); // Fetch and store requests for the current user
?>
<!DOCTYPE html>
<html lang="en">
		...
		<div class="container">
            ...
            if ($requests->num_rows != 0){
                foreach ($requests as $user_block){ // Loop through the requests and display user blocks for each request
                    ...
                    print_user_block_request($user->get_full_name(), $user->get_avatar($conn), $user->get_id()); // Print a user block for the request
                }
