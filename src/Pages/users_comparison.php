...
if ($is_valid1 && $is_valid2){
    ...
    $user1_workouts = $user1->get_control_workouts($conn, 1, NULL);
    $user2_workouts = $user2->get_control_workouts($conn, 1, NULL);
	...
	<main class="user-comparison">
				...
				<p class="staff-block__title">Первый спортсмен</p>
                    <?php if ($user1 == NULL){ // if the user is not selected ?>
                        <section class="staff-block__header">
                            <button class="button-text comparison-block__add-button comparison-block__add-button--first"><p>Добавить спортсмена</p> <img src="../../assets/img/add.svg" alt=""></button>
                        </section>
                    <?php } else {
                        ...
                        if ($flag_main){ // If flag_main is true (indicating similarity in last exercises between users)
                            foreach ($last_1 as $item){
                                ...
                                foreach ($item->muscles as $muscle){ // Generate muscle list
                                    $muscle_list .= translate_group($muscle) . " users_comparison.php";
                                }
                                ...
                                $reps["{{ exercises }}"] .= render($replaces, BASE_PATH . "templates/control_exercise.html"); // Render exercise details and concatenate to existing exercises data
                            }
                        }
                        echo render($reps, BASE_PATH . "templates/comparison_block.html");  // Render the comparison block with exercise details
                    } ?>
			</section>
			<section class="comparison-block">
				<p class="staff-block__title">Второй спортсмен</p>
                    <?php if ($user2 == NULL){ // if the user is not selected ?>
                        <section class="staff-block__header">
                            <button class="button-text comparison-block__add-button comparison-block__add-button--second"><p>Добавить спортсмена</p> <img src="../../assets/img/add.svg" alt=""></button>
                        </section>
                    <?php } else {
                        if ($is_valid1)
                        ...
                        if ($flag_main){
                            foreach ($last_2 as $item){
                                ...
                                $reps["{{ exercises }}"] .= render($replaces, BASE_PATH . "templates/control_exercise.html");
                            }
                        }
                        echo render($reps, BASE_PATH . "templates/comparison_block.html");
                    } ?>
			</section>
		</div>
        ...
        <section class="popup-exercise popup-exercise--user-first">
			<form class="popup-exercise__content popup-exercise--add-users__form">
				<button type="button" type="button" class="popup-exercise__close-button"><img src="../../assets/img/close.svg" alt=""></button>
                    ...
                    <?php } ?>
				<button type="submit" class="button-text popup-exercise--add-users__button-add" type="submit"><p>Добавить</p><img src="../../assets/img/add.svg" alt=""></button>
			</form>
		</section>
